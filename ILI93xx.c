...
{										   
	...
	LCD_RD_CLR;
	if(lcddev.id==0X8989)
		//delay_us(2);replaced by Delay 1//FOR 8989,延时2us	
		Delay(1);
	t=(u16)DATAIN  
	LCD_RD_SET;
	LCD_CS_SET; 
...
// 	printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID  
	...
	GPIOC->ODR&=0xFDFF;//LCD_LED=1;					//点亮背光
	LCD_Clear(WHITE);
}  		  
...
//fill a imge in a lcd module
void LCD_Fill_img(u16 xsta,u16 ysta,u16 xend,u16 yend,unsigned char *p)
{
	u16 i,j,dhl;
	u8 dh,dl;
	u16 xlen=0;
